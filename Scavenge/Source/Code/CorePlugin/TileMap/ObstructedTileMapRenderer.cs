...
namespace Scavenge.TileMap
{
    ...
    public class ObstructedTilemapRenderer : Renderer, ICmpTilemapRenderer
    {
        ...
        private bool ForceShow
        {
            get
            {
                return !DoObstructedRendering || (AlwaysShowInEditor && DualityApp.ExecEnvironment == DualityApp.ExecutionEnvironment.Editor);
            }
        }
        ...
        public TileMapObstructionBase Obstruction
        {
            get
            {
                ...
                else
                {
                    ...
                    {
                        _internalObstruction = GameObj.GetComponent<TileMapObstructionBase>();
                    }
        ...
        public override void Draw(IDrawDevice device)
        {
            ...
            for (int tileIndex = 0; tileIndex < renderedTileCount; tileIndex++)
            {
                ...
                if(Obstruction != null && !ForceShow)
                {
                    visible = !Obstruction.IsObstructed(tileGridPos);
                }

                if (visible && tile.Index < tileData.Length)
                {
            ...
            Profile.AddToStat(@"Scavenge\Stats\Render\Tilemaps\NumTiles", renderedTileCount);
            Profile.AddToStat(@"Scavenge\Stats\Render\Tilemaps\NumVertices", submittedTileCount * 4);
        }
    }
}